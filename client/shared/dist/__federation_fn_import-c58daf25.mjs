...
function ue(e) {
  return e.replace(
    o(J),
    (t, s, r, n, i, $, u, a, f, l, R, p) => (c(r) ? s = "" : c(n) ? s = `>=${r}.0.0` : c(i) ? s = `>=${r}.${n}.0` : s = `>=${s}`, c(f) ? a = "" : c(l) ? a = `<${+f + 1}.0.0-0` : c(R) ? a = `<${f}.${+l + 1}.0-0` : p ? a = `<=${f}.${l}.${R}-${p}` : a = `<=${a}`, `${s} ${a}`.trim())
  );
}
function ce(e) {
...
function le(e) {
  return e.trim().split(/\s+/).map((t) => t.replace(
    o(ee),
    (s, r, n, i, $) => c(r) ? "" : c(n) ? `>=${r}.0.0 <${+r + 1}.0.0-0` : c(i) ? r === "0" ? `>=${r}.${n}.0 <${r}.${+n + 1}.0-0` : `>=${r}.${n}.0 <${+r + 1}.0.0-0` : $ ? r === "0" ? n === "0" ? `>=${r}.${n}.${i}-${$} <${r}.${n}.${+i + 1}-0` : `>=${r}.${n}.${i}-${$} <${r}.${+n + 1}.0-0` : `>=${r}.${n}.${i}-${$} <${+r + 1}.0.0-0` : r === "0" ? n === "0" ? `>=${r}.${n}.${i} <${r}.${n}.${+i + 1}-0` : `>=${r}.${n}.${i} <${r}.${+n + 1}.0-0` : `>=${r}.${n}.${i} <${+r + 1}.0.0-0`
  )).join(" ");
}
function oe(e) {
  return e.trim().split(/\s+/).map((t) => t.replace(
    o(te),
    (s, r, n, i, $) => c(r) ? "" : c(n) ? `>=${r}.0.0 <${+r + 1}.0.0-0` : c(i) ? `>=${r}.${n}.0 <${r}.${+n + 1}.0-0` : $ ? `>=${r}.${n}.${i}-${$} <${r}.${+n + 1}.0-0` : `>=${r}.${n}.${i} <${r}.${+n + 1}.0-0`
  )).join(" ");
}
function pe(e) {
  return e.split(/\s+/).map((t) => t.trim().replace(
    o(se),
    (s, r, n, i, $, u) => {
      const a = c(n), f = a || c(i), l = f || c($);
      return r === "=" && l && (r = ""), u = "", a ? r === ">" || r === "<" ? "<0.0.0-0" : "*" : r && l ? (f && (i = 0), $ = 0, r === ">" ? (r = ">=", f ? (n = +n + 1, i = 0, $ = 0) : (i = +i + 1, $ = 0)) : r === "<=" && (r = "<", f ? n = +n + 1 : i = +i + 1), r === "<" && (u = "-0"), `${r + n}.${i}.${$}${u}`) : f ? `>=${n}.0.0${u} <${+n + 1}.0.0-0` : l ? `>=${n}.${i}.0${u} <${n}.${+i + 1}.0-0` : s;
    }
  )).join(" ");
}
function de(e) {
...
function Re(e, t) {
  ...
  for (let n = 0, i = s.length; n <= i; n++) {
    const $ = s[n], u = r[n];
    if ($ !== u)
      return $ === void 0 && u === void 0 ? 0 : $ ? u ? h($, u) : -1 : 1;
  }
  return 0;
}
function V(e, t) {
...
const Z = {}, x = { react: { get: () => () => k(new URL("__federation_shared_react-9351769d.mjs", import.meta.url).href), import: !0 }, "react-dom": { get: () => () => k(new URL("__federation_shared_react-dom-c9e99da3.mjs", import.meta.url).href), import: !0 } }, w = /* @__PURE__ */ Object.create(null);
...
async function be(e, t) {
  let s = null;
  if (globalThis?.__federation_shared__?.[t]?.[e]) {
    const r = globalThis.__federation_shared__[t][e], n = x[e]?.requiredVersion;
    if (!!n) {
      const $ = Object.keys(r).find(
        (u) => we(u, n)
      );
      $ ? s = await (await r[$].get())() : console.log(
        `provider support ${e}(${$}) is not satisfied requiredVersion(\${moduleMap[name].requiredVersion})`
      );
    } else {
